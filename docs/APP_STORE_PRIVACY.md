...
## 6. 출시 전 체크리스트
- [x] **앱 내 개인정보처리방침 접근**(5.1.1(i) "within the app") — `privacy.html` 번들 임베딩 + 마이 → 개인정보처리방침 WebView (#66)
- [ ] PRIVACY.md를 공개 URL로 호스팅(GitHub Pages 등) → App Store Connect **Privacy Policy URL**에 입력 (5.1.1(i) "App Store Connect" 부분)
- [ ] **AI 해석 최초 사용 시 명시적 동의 모달** — 5.1.2(i)가 제3자 AI 전송 전 explicit permission 요구 → *필수*. "생년월일시·성별·출생지가 해석 생성을 위해 OpenAI로 전송됨" 1회 동의
- [ ] App Privacy 설문을 위 표대로 작성·제출
- [ ] (권장) OpenAI 데이터 처리 정책을 PRIVACY.md 작성 시점 기준으로 재확인
- [ ] (권장) App Review Notes에 4.3 차별성 근거 기재(천체력 자체구현·한국 사주 특화·진태양시 글로벌 보정)
- [ ] 운영자/개인정보 보호책임자 정보·문의 이메일 최종 확정

> 심사 규정 근거: App Review Guidelines 5.1.1(개인정보처리방침 필수·앱내+ASC URL), 5.1.2(제3자 AI 전송 명시 동의), 1.5(연락처), 4.3(점술 차별성). 이용약관/EULA는 구독·계정 없으므로 의무 아님(미작성 시 Apple 표준 EULA 적용). 상세: WORKLOG #66.
