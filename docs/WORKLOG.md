...
- **브랜치**: main
- **최신 빌드**: 성공 (2026-07-02, iPhone 15). 최근: **3D 타로 뽑기 POC**(부채꼴·펼침·플립 공개, 원카드, #67, DEC-021)·개인정보처리방침 인앱 임베딩 + 앱스토어 심사 규정 조사(#66)·마이 정보 표기+보안 감사(#65)·글로벌 출생지 지원(세계 87도시, #64, DEC-020)·운세 달력 UI 개선(#63)
...
#### 67. 3D 타로 뽑기 POC — 부채꼴 펼침 + 뽑기 + 고급 공개 (2026-07-02, 원카드)
- **요청**: 타로를 3D로 뽑는 POC — 버드뷰 부채꼴 뒷면 → 터치하면 "사사삭"(카드소리) 펼침 → 한 장 선택 → 뽑힌 카드가 풀스크린 고급 화면
- **설계 방향(UX+iOS)**: SwiftUI 2.5D vs SceneKit 비교 → **SwiftUI 2.5D 채택**(평면 카드는 안무된 모션이 적합, 기존 에셋·플립 재사용). 원카드 풀플로우부터. **DEC-021**
- **무대(`TarotTableView`)**: 딜-인(순차 날아옴) → 부채꼴 겹침(stacked) → 탭 펼침(fanned, riffle 사운드+햅틱) → 좌우 드래그 훑기 → 선택 → `matchedGeometry`로 카드가 중앙으로 뽑혀 나옴. 펠트 테이블 배경 + 바닥 타원 그림자
- **공개(`CardRevealView`)**: 뒷면→앞면 3D 플립, `CoreMotion` 자이로 기울임에 광택(gloss) 흐름, 부유/그림자/✨별빛, 위치·한영 이름·역방향 배지·키워드 칩 + 기존 AI 타로 리딩 연결
- **사운드(`CardSound`)**: 라이선스 회피 위해 **직접 합성**(파이썬→afconvert AAC) `card-slide`/`card-riffle`. 없으면 CoreHaptics 폴백, 무음 스위치 존중(`.ambient`)
- **결정성**: 카드는 기존 `drawCards`(결정적 시드)가 먼저 결정, 뒷면 동일 → 어느 걸 골라도 결정된 카드 배정(공정성)
- **접근성**: reduce motion 시 기존 `TarotReadingView` 그리드로 폴백
- **튜닝(시뮬 캡처 반복)**: 부채꼴 위치 0.6→0.48, step 7°→5.4°·피벗 2.4→2.0(양끝 미절단), 문구 부채꼴 하단, 바닥 그림자 추가. BUILD SUCCEEDED. (자이로 광택은 실기기 확인 필요)
- **파일**: `App/Views/Tarot/Draw3D/{TarotTableView,CardRevealView,CardSound}.swift`(신설)·`App/Views/Tarot/TarotView.swift`(원카드 분기)·`App/Resources/card-{slide,riffle}.m4a`(신설)
- **인터랙션 재설계(사용자 실기 피드백 반영)**: ① 시작 시 카드가 **왼쪽 더미**로 쌓임 → 드래그로 부채꼴 펼침(손가락 따라 점진적) ② 펼친 뒤 훑기 = 손가락 근처 카드가 **y로만 파도처럼** 올라옴(겹침 z 고정, 지나가면 내려감) ③ 선택 = 카드를 **위로 밀어 뽑기**(pull-out) — 누른 카드가 y로 올라오고 **이웃은 x로만 벌어짐**(z 순서 불변), 임계 넘으면 확정, 덜 밀면 스냅백. 꾹 누르기도 선택. ④ 훑기/뽑기는 **드래그 시작 방향으로 모드 고정**(가로=훑기, 위=뽑기)해 오선택 방지, 대상은 `cardAt`(겹침 최상단 카드)로 정확화
- **공개 연출 3종 + 랜덤**: 선택→플립 공개 장면을 ① 의식형(금빛 방사 광선+오라+별가루 버스트) ② 시네마틱(암전·비네트 스포트라이트+느린 overshoot 플립+흰 섬광 스윕+골드 글로우) ③ 토스(다축 회전+스케일 펀치) 3종으로 구현, **공개할 때마다 랜덤 재생**(`RevealStyle.allCases.randomElement()`, 뷰 재생성마다 재추첨). 카드 결과는 결정적·연출만 랜덤
- **후속**: 쓰리카드/켈틱 다장 확장, 실기기 자이로·사운드 확인, 프로 사운드 샘플 교체, 연출 수치 미세조정(선택)
...
- **표기**: 생년월일 항상 `양력/음력` 표기(이전 음력만), 출생지 `도시 · 국가`(예: 도쿄 · 일본), 피커 행에 국가 부제. `RegionCoords.City`에 표시 전용 `country` 87개 추가(계산 미사용·서버 미전송)
- **보안 감사(코드 실측)**: ① 로컬 — 프로필은 `UserDefaults`(앱 샌드박스+iOS 데이터보호), **이름은 로컬에만** ② 전송 — `daltokkie.vercel.app` 단일 HTTPS, 성별·생년월일시·지역·계산차트만(이름·기기ID·GPS·토큰 미전송) ③ 서버저장 — saju-api **완전 stateless**(DB/KV/파일/분석SDK 전무) ④ 제3자 — AI 해석 시 생년월일시·성별·지역이 **OpenAI API**로 전달(키는 서버 env)
- **정직성 보완**: 마이 하단 문구를 "AI 심층 해석을 볼 때만 생년월일시·성별·출생지가 서버로 전송(이름·기기·위치 미전송·미저장)"으로 수정
- **남은 권고**: App Store 제출용 개인정보처리방침 문서화(별도)
- **파일**: `App/Models/UserProfile.swift`(country), `App/Views/Misc/TalismanMyViews.swift`, `App/Views/Misc/RegionPicker.swift`
...
- **음력**: 셀마다 음력일 표시(초하루 "5월 1일", 보름 "보름", 일반 "음 13"), 탭 상세에 "음력 5.15"
- **절기(24절기)**: `SolarTermsTable.termsInMonth(year:month:)` 공개 API 추가 → 망종·하지 등 표시. **버그 수정**: 소한·대한이 데이터상 전년도로 저장돼 1월 누락 → 인접 연도(year±1) 스캔으로 24절기 전부
- **명절(음력)**: 단오(절기 아님!)·설날·정월대보름·삼짇날·유두·칠석·백중·추석·중양절 — 음력 월/일 기준 별도 레이어
- **달 위상**: 음력일 기반 **Canvas 직접 렌더**(`MoonPhase`) — 그림자 원반+밝은 반원+터미네이터. SF moonphase 심볼이 작은 크기서 부정확(보름 검게)해 교체. 노르스름한 달빛 색
- **가독성**: 주말 색(일 빨강·토 파랑), 점수 히트맵 톤, 점수 점→칸 우상단 코너, **오늘만 박스 강조**(채워진 원), 절기·명절은 날짜 숫자 색(절기 핑크·명절 빨강)으로, **범례(오늘·명절·절기·음력 + 점수)** 달력 하단
- **검증**: 빌드 ✅, TEMP 풀스크린 캡처로 단오(6/19)·소한(1/5)·달 위상·오늘 강조·"5월 1일" 확인. 앱+엔진 전용(서버 재배포 불필요)

#### 62. daily 콘텐츠 일관성·다양성·정확도 마감 + 출력 레벨 검증 (2026-06-28)
- **배경**: 사용자가 실제 앱에서 연속 지적 — ① 홈 오늘의운세 ≠ 사주 오늘의운세, ② 한마디/운세/달빛편지 서로 모순, ③ 매일 직장운/재물운만, ④ 일요일에 "상사와 대화", ⑤ 행운포인트 시간대가 화면 카드와 불일치
- **근본 원인**: (a) 독립 LLM 호출 + 캐시 없음 → 같은 콘텐츠도 화면마다 다름, (b) 엔진 권위값(행운시간/색/방위/숫자·일진·달력)을 LLM에 안 줘 추측, (c) 엔진 점수가 직장/재물에 쏠려 '1등 영역'이 매일 직장/재물, (d) 요일/주말 무시, (e) **홈 달빛편지는 `/api/daily/interpret` 별도 경로라** content 경로에 넣은 앵커가 누락
- **수정**:
  - **행운포인트**: 시간대·색상(엔진 `luckyHours`/`luckyItems`) + 방위·숫자(용신 오행 결정값, LLM 매핑 오류 제거) 주입 → 화면 카드와 일치
  - **일관성 캐시**: daily 콘텐츠를 (id·날짜·프로필) 캐시(`cachedDailyStream`) + 톤 warm 고정 → 홈 자세히보기 ↔ 사주 세부해석 동일 텍스트
  - **영역 다양화**: `AppState.dailyAreas` — 1등만 쓰지 않고 상위 3개 중 날짜 시드로 회전, **주말엔 직장·학업 제외**. content·`interpretDaily` 두 경로 모두 적용. 캐시 v4
  - **주말 인지**: 두 경로 프롬프트에 "주말이면 업무 조언 금지" + isWeekend 전달
  - **lucky-day**: 이번 달+다음 달 달력 주입 + "달력 날짜만, 오늘 이후만" → 7월 간지 환각·지난 날짜 추천 제거
  - daily-one-liner grounded 프롬프트로 교체(단, **BASE_KNOWLEDGE가 장문 강제해 아직 미반영** — 후속 과제)
- **출력 레벨 검증(이전 컴파일 검증의 한계 인정 후 도입)**: 배포된 서버를 **직접 curl 호출**해 사실값 대조. 48종 전수 확인 — 점성(태양 쌍둥이자리 등)·자미(생년사화·성요)·연도(2026 기준)·트랜짓·행운값 모두 엔진값 일치. **남은 이슈: daily-one-liner 형식(BASE)** 1건
- **배포**: `vercel --prod` 직접 수행(사용자 승인). 앱 7일 캐시 덤프로 영역 다양·주말 반영 실측 확인
- **관련 커밋**: iOS·saju-api 다수 (행운포인트·방위숫자·일관성·다양화·lucky-day). DEC-019 참조
