...
| AI 해석 | LLM은 엔진 계산값만 해석. 사주=서버 `ftFullAnalysis` 재계산, 자미=생년사화·양력보정, 점성=실제 트랜짓 주입 → **환각 차단**(WORKLOG #60·61) |
| 시점 | `nowContext`/`buildDailyJinContext`로 현재 연도·오늘 일진 강제 — 연도/일진 환각 방지 |
| 의도된 예외 | 일일운세 점수 알고리즘은 앱 자체 진화(DEC-014, saju-api 비트재현 아님) |
